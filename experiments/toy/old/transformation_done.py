...
class Simplex(InputTransform, Module):
    r"""A simplex transformation done to the input points to optimize Simplex spaces 
    instead of random variables whose space lies in a hypercube."".
    ...
    def transform(self, X: Tensor) -> Tensor:
        ...
        Returns:
            A `batch_shape x n x d`-dim tensor of rounded inputs.
        """
        #X_int = X_rounded[..., self.indices] Para pasar solo las que quieres.
        instances = X.shape[0]
        dimensions = 0
        if len(X.shape)==2:
        ...
        if len(X.shape)==2:
            norm_constants = torch.sum(torch.exp(transformed_inputs), axis=1).repeat_interleave(dimensions).reshape(instances, dimensions)
        else:
            norm_constants = torch.sum(torch.exp(transformed_inputs), axis=2).repeat_interleave(dimensions).reshape(instances, 1, dimensions)
        X_rounded = torch.exp(transformed_inputs)/norm_constants
        #X_rounded[..., self.indices] = X_int Para pasar solo las que quieres.
        """
        X_rounded = X.clone()
        X_int = X_rounded[..., self.indices]
        if self.approximate:
